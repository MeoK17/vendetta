...
mp.events.add('character:CreatorCamera', () => {
	if (skycam) {
		...
		setTimeout(() => {
			...
			bodyCam = mp.cameras.new('default', creatorCoords.camera, new mp.Vector3(0,0,0), 70);
...
function checkCamInAirCharSelectChar() {
    if (mp.game.invoke(Natives.IS_PLAYER_SWITCH_IN_PROGRESS)) {
       ...
       if(editorBrowser == null) editorBrowser = mp.browsers.new('http://package/browser/character/index.html#content-1');
		global.menuOpen();
		skycam = false;
    }
}
