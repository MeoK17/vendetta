...
mp.events.add("auth:accountAuth", (player, data) => {
    ...
    DB.query('SELECT * FROM accounts WHERE login = ? LIMIT 1', [data.login], function (err, results){
        if(results.length == 0) return player.call('notify:pushNotify', [1, 8, 'Неверный Логин или Пароль', 3000]);
        if(results[0].socialclub != player.socialClub) return player.call('notify:pushNotify', [1, 8, 'Этот аккаунт привязан к другому SocialClub', 3000]);

        const dbPassword = results[0].password;
        bcrypt.compare(data.password, dbPassword).then(async function(isMatched) {
            if(!isMatched) return player.call('notify:pushNotify', [1, 8, 'Неверный Логин или Пароль', 3000]);
...
mp.events.add("auth:accountRegistration", (player, data) => {
    ...
    DB.query('SELECT * FROM accounts WHERE login = ?', [data.login], function (err, results){
        console.log(results)
        if(results.length > 0) return player.call('notify:pushNotify', [1, 8, 'Аккаунт с таким Логином уже существует', 3000]);
        DB.query('SELECT id FROM accounts WHERE email = ?', [data.email], function (err, results){
            if(results.length > 0) return player.call('notify:pushNotify', [1, 8, 'Аккаунт с такой Почтой уже существует', 3000]);
            DB.query('SELECT id FROM accounts WHERE socialclub = ?', [player.socialClub], function (err, results){
                if(results.length > 0) return player.call('notify:pushNotify', [1, 8, 'Аккаунт с таким SocialClub уже существует', 3000]);
